...
class CharList(
    ...
    inline fun forEachIndexed(block: (Int, Char) -> Unit) {
        for (i in 0 until size) {
            block(i, array[i])
        }
    }

    inline fun find(block: (Char) -> Boolean): Char? {
        for (i in 0 until size) {
            val value = array[i]
            if (block(value)) {
                return value
    ...
    inline fun findIndex(block: (Char) -> Boolean): Int {
        for (i in 0 until size) {
            if (block(array[i])) {
                return i
            }
    ...
    inline fun filter(block: (Char) -> Boolean): CharList {
        val result = CharList(size)
    ...
    inline fun sortBy(crossinline selector: (Char) -> Int) {
        sortWith { a, b ->
            selector(a).compareTo(selector(b))
        }
    }

    fun sortedWith(comparator: (Char, Char) -> Int): CharList =
        clone().apply {
            sortWith(comparator)
        }

    inline fun sortedBy(crossinline selector: (Char) -> Int): CharList =
        clone().apply {
            sortBy(selector)
        }
