...
inline fun LongList(
    capacity: Int = 16,
    noinline init: (Int) -> Long = { 0L }
) = LongList(0, LongArray(capacity, init))
...
class LongList(
   ...
   var array: LongArray,
    ...
    inline fun clone(): LongList {
        return LongList(size, array.copyOf())
    }
    ...
    inline fun filterWith(block: (Long) -> Boolean): LongList {
        var writeIndex = 0
    ...
    inline fun sortByInt(crossinline selector: (Long) -> Int) {
        if (size < 2) return
    ...
    inline fun sortByLong(crossinline selector: (Long) -> Long) {
        if (size < 2) return
        val stack = __sortStack ?: LongArray(64).also { __sortStack = it }
        var stackSize = 0
        stack[stackSize++] = (size - 1).toLong() and 0xffffffffL
    ...
    inline fun sortByFloat(crossinline selector: (Long) -> Float) {
        if (size < 2) return
        val stack = __sortStack ?: LongArray(64).also { __sortStack = it }
        var stackSize = 0
        stack[stackSize++] = (size - 1).toLong() and 0xffffffffL
    ...
    inline fun sortByDouble(crossinline selector: (Long) -> Double) {
        if (size < 2) return
        val stack = __sortStack ?: LongArray(64).also { __sortStack = it }
        var stackSize = 0
        stack[stackSize++] = (size - 1).toLong() and 0xffffffffL
...
inline fun LongList.encode(i: Int, buffer: NativeBuffer) = buffer.pushLong(array[i])
inline fun LongList.encodeGpu(i: Int, buffer: NativeBuffer) = buffer.pushLong(array[i])
inline fun LongList.encodePacked(i: Int, buffer: NativeBuffer) = buffer.pushLong(array[i])

inline fun LongList.decode(i: Int, buffer: NativeBuffer) { array[i] = buffer.nextLong() }
inline fun LongList.decodeGpu(i: Int, buffer: NativeBuffer) { array[i] = buffer.nextLong() }
inline fun LongList.decodePacked(i: Int, buffer: NativeBuffer) { array[i] = buffer.nextLong() }
