...
import com.cws.extra.memory.nextBoolean
import com.cws.extra.memory.nextFloat
import com.cws.extra.memory.pushBoolean
...
) = BooleanList(0, BooleanArray(capacity, init))
...
class BooleanList(
    ...
    inline fun clone(): BooleanList {
        return BooleanList(size, array.copyOf())
    }

    inline fun forEach(block: (Boolean) -> Unit) {
        for (i in 0 until size) {
            block(array[i])
        }
    }

    inline fun forEachIndexed(block: (Int, Boolean) -> Unit) {
        for (i in 0 until size) {
            block(i, array[i])
        }
    }

    inline fun find(block: (Boolean) -> Boolean): Boolean? {
        for (i in 0 until size) {
            val value = array[i]
            if (block(value)) {
                return value
    ...
    inline fun findIndex(block: (Boolean) -> Boolean): Int {
        for (i in 0 until size) {
            if (block(array[i])) {
                return i
            }
    ...
    inline fun filter(block: (Boolean) -> Boolean): BooleanList {
        val result = BooleanList(size)
    ...
    inline fun filterWith(block: (Boolean) -> Boolean): BooleanList {
        var writeIndex = 0
    ...
    inline fun sortByInt(crossinline selector: (Boolean) -> Int) {
        if (size < 2) return
    ...
    inline fun sortByLong(crossinline selector: (Boolean) -> Long) {
        if (size < 2) return
        val stack = __sortStack ?: LongArray(64).also { __sortStack = it }
        var stackSize = 0
        stack[stackSize++] = (size - 1).toLong() and 0xffffffffL
    ...
    inline fun sortByFloat(crossinline selector: (Boolean) -> Float) {
        if (size < 2) return
        val stack = __sortStack ?: LongArray(64).also { __sortStack = it }
        var stackSize = 0
        stack[stackSize++] = (size - 1).toLong() and 0xffffffffL
    ...
    inline fun sortByDouble(crossinline selector: (Boolean) -> Double) {
        if (size < 2) return
        val stack = __sortStack ?: LongArray(64).also { __sortStack = it }
        var stackSize = 0
        stack[stackSize++] = (size - 1).toLong() and 0xffffffffL
    ...
    fun sortWith(comparator: (Boolean, Boolean) -> Int) {
    ...
    inline fun sortBy(crossinline selector: (Boolean) -> Int) {
        sortWith { a, b ->
            selector(a).compareTo(selector(b))
        }
    }

    fun sortedWith(comparator: (Boolean, Boolean) -> Int): BooleanList =
        clone().apply {
            sortWith(comparator)
        }

    inline fun sortedBy(crossinline selector: (Boolean) -> Int): BooleanList =
        clone().apply {
            sortBy(selector)
        }
...
inline fun BooleanList.encode(i: Int, buffer: NativeBuffer) = buffer.pushBoolean(array[i])
inline fun BooleanList.encodeGpu(i: Int, buffer: NativeBuffer) = buffer.pushBoolean(array[i])
inline fun BooleanList.encodePacked(i: Int, buffer: NativeBuffer) = buffer.pushBoolean(array[i])

inline fun BooleanList.decode(i: Int, buffer: NativeBuffer) { array[i] = buffer.nextBoolean() }
inline fun BooleanList.decodeGpu(i: Int, buffer: NativeBuffer) { array[i] = buffer.nextBoolean() }
inline fun BooleanList.decodePacked(i: Int, buffer: NativeBuffer) { array[i] = buffer.nextBoolean() }
