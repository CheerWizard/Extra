...
) = IntList(0, IntArray(capacity, init))
...
data class IntList(
    ...
    inline fun clone(): IntList {
        return IntList(size, array.copyOf())
    }
    ...
    /** Retains matching values in-place without allocating and preserves their order. */
    inline fun filterWith(block: (Int) -> Boolean): IntList {
        var writeIndex = 0
    ...
    inline fun sortByInt(crossinline selector: (Int) -> Int) {
        if (size < 2) return
    ...
    inline fun sortByLong(crossinline selector: (Int) -> Long) {
        if (size < 2) return
        val stack = __sortStack ?: LongArray(64).also { __sortStack = it }
        var stackSize = 0
        stack[stackSize++] = (size - 1).toLong() and 0xffffffffL
    ...
    inline fun sortByFloat(crossinline selector: (Int) -> Float) {
        if (size < 2) return
        val stack = __sortStack ?: LongArray(64).also { __sortStack = it }
        var stackSize = 0
        stack[stackSize++] = (size - 1).toLong() and 0xffffffffL
    ...
    inline fun sortByDouble(crossinline selector: (Int) -> Double) {
        if (size < 2) return
        val stack = __sortStack ?: LongArray(64).also { __sortStack = it }
        var stackSize = 0
        stack[stackSize++] = (size - 1).toLong() and 0xffffffffL
...
inline fun IntList.encode(i: Int, buffer: NativeBuffer) = buffer.pushInt(array[i])
inline fun IntList.encodeGpu(i: Int, buffer: NativeBuffer) = buffer.pushInt(array[i])
inline fun IntList.encodePacked(i: Int, buffer: NativeBuffer) = buffer.pushInt(array[i])

inline fun IntList.decode(i: Int, buffer: NativeBuffer) { array[i] = buffer.nextInt() }
inline fun IntList.decodeGpu(i: Int, buffer: NativeBuffer) { array[i] = buffer.nextInt() }
inline fun IntList.decodePacked(i: Int, buffer: NativeBuffer) { array[i] = buffer.nextInt() }
