...
import com.cws.extra.math.operators.sqrt
...
data class UInt4(
    ...
    operator fun plus(v: UInt): UInt4 = UInt4(x + v, y + v, z + v, w + v)

    operator fun minus(v: UInt): UInt4 = UInt4(x - v, y - v, z - v, w - v)

    operator fun times(v: UInt): UInt4 = UInt4(x * v, y * v, z * v, w * v)

    operator fun div(v: UInt): UInt4 = UInt4(x / v, y / v, z / v, w / v)

    operator fun plus(v: UInt4): UInt4 = UInt4(x + v.x, y + v.y, z + v.z, w + v.w)

    operator fun minus(v: UInt4): UInt4 = UInt4(x - v.x, y - v.y, z - v.z, w - v.w)

    operator fun times(v: UInt4): UInt4 = UInt4(x * v.x, y * v.y, z * v.z, w * v.w)

    operator fun div(v: UInt4): UInt4 = UInt4(x / v.x, y / v.y, z / v.z, w / v.w)
