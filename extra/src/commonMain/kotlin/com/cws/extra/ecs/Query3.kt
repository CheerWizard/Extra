...
/**
 * A query for entities that have components of type [T1], [T2], and [T3].
 */
class Query3<
    T1, L1 : IExtraList,
    T2, L2 : IExtraList,
...
>(
    ...
    init {
        ...
        dp = when {
            p1.size <= p2.size && p1.size <= p3.size -> p1
            p2.size <= p3.size -> p2
            else -> p3
        }
        __size = dp.size
    ...
    internal fun refreshIfNeeded() {
        if (__version1 == p1.structuralVersion && __version2 == p2.structuralVersion && __version3 == p3.structuralVersion) return
        refresh()
    }
    ...
    fun refresh(): Query3<T1, L1, T2, L2, T3, L3> {
        __version1 = p1.structuralVersion
        __version2 = p2.structuralVersion
        __version3 = p3.structuralVersion
        dp = when {
    ...
    inline fun <reified T4, reified L4 : IExtraList> with() =
        Query4<T1, L1, T2, L2, T3, L3, T4, L4>(scene, id1, id2, id3, ComponentId<T4>())

    fun without1(): Query2<T2, L2, T3, L3> = Query2(scene, id2, id3)
    fun without2(): Query2<T1, L1, T3, L3> = Query2(scene, id1, id3)
    fun without3(): Query2<T1, L1, T2, L2> = Query2(scene, id1, id2)

    inline fun filter(predicate: (entity: Int, c1: L1, i1: Int, c2: L2, i2: Int, c3: L3, i3: Int) -> Boolean): Query3<T1, L1, T2, L2, T3, L3> {
        val c1 = c1
        val c2 = c2
        val c3 = c3
        ensureIndices().filterWith { i ->
            val entity = __entities[i]
            predicate(entity, c1, componentIndex(p1, __e1, entity, i), c2, componentIndex(p2, __e2, entity, i), c3, componentIndex(p3, __e3, entity, i))
        }
        return this
    }

    inline fun <R : Comparable<R>> sortedBy(crossinline selector: (entity: Int, c1: L1, i1: Int, c2: L2, i2: Int, c3: L3, i3: Int) -> R): Query3<T1, L1, T2, L2, T3, L3> {
        ...
        list.sortWith { iA, iB ->
            ...
            selector(entityA, c1, i1A, c2, i2A, c3, i3A).compareTo(selector(entityB, c1, i1B, c2, i2B, c3, i3B))
        }
        return this
    }


    inline fun sortedByInt(crossinline selector: (entity: Int, c1: L1, i1: Int, c2: L2, i2: Int, c3: L3, i3: Int) -> Int): Query3<T1, L1, T2, L2, T3, L3> {
        val list = ensureIndices()
        val c1 = c1
        val c2 = c2
        val c3 = c3
        list.sortByInt { i ->
            ...
            val i2 = componentIndex(p2, __e2, entity, i)
            val i3 = componentIndex(p3, __e3, entity, i)
            selector(entity, c1, i1, c2, i2, c3, i3)
        }
        return this
    }

    inline fun sortedByLong(crossinline selector: (entity: Int, c1: L1, i1: Int, c2: L2, i2: Int, c3: L3, i3: Int) -> Long): Query3<T1, L1, T2, L2, T3, L3> {
        val list = ensureIndices()
        val c1 = c1
        val c2 = c2
        val c3 = c3
    ...
    inline fun sortedByFloat(crossinline selector: (entity: Int, c1: L1, i1: Int, c2: L2, i2: Int, c3: L3, i3: Int) -> Float): Query3<T1, L1, T2, L2, T3, L3> {
        val list = ensureIndices()
        val c1 = c1
        val c2 = c2
        val c3 = c3
    ...
    inline fun sortedByDouble(crossinline selector: (entity: Int, c1: L1, i1: Int, c2: L2, i2: Int, c3: L3, i3: Int) -> Double): Query3<T1, L1, T2, L2, T3, L3> {
        val list = ensureIndices()
        val c1 = c1
        val c2 = c2
        val c3 = c3
    ...
    fun reverse(): Query3<T1, L1, T2, L2, T3, L3> {
        val list = ensureIndices()
        var i = 0
        var j = list.size - 1
        while (i < j) {
    ...
    fun take(n: Int): Query3<T1, L1, T2, L2, T3, L3> {
    ...
    inline fun first(
        ...
        for (idx in 0 until size) {
            ...
            val i3 = componentIndex(p3, __e3, entity, i)
            if (i1 != ComponentNull && i2 != ComponentNull && i3 != ComponentNull) {
                block(entity, c1, i1, c2, i2, c3, i3)
                break
            }
        }
        return this
    ...
    @PublishedApi
    internal inline fun forEachUnfiltered(block: (entity: Int, c1: L1, i1: Int, c2: L2, i2: Int, c3: L3, i3: Int) -> Unit): Query3<T1, L1, T2, L2, T3, L3> {
        val indices = __indices
        val c1 = c1
        val c2 = c2
        val c3 = c3
    ...
    inline fun forEachWhere(
        predicate: (entity: Int, c1: L1, i1: Int, c2: L2, i2: Int, c3: L3, i3: Int) -> Boolean,
        block: (entity: Int, c1: L1, i1: Int, c2: L2, i2: Int, c3: L3, i3: Int) -> Unit,
    ): Query3<T1, L1, T2, L2, T3, L3> {
        forEach { entity, c1, i1, c2, i2, c3, i3 ->
            if (predicate(entity, c1, i1, c2, i2, c3, i3)) {
                block(entity, c1, i1, c2, i2, c3, i3)
            }
        }
        return this
    ...
    inline fun all(predicate: (entity: Int, c1: L1, i1: Int, c2: L2, i2: Int, c3: L3, i3: Int) -> Boolean): Boolean {
        forEach { entity, c1, i1, c2, i2, c3, i3 ->
            if (!predicate(entity, c1, i1, c2, i2, c3, i3)) return false
        }
        return true
    }
    ...
    inline fun last(): Int {
        ...
        forEach { entity, _, _, _, _, _, _ -> result = entity }
        return result
    }
    ...
    inline fun any(): Boolean {
        ...
        for (idx in 0 until size) {
            ...
            val i2 = componentIndex(p2, __e2, entity, i)
            val i3 = componentIndex(p3, __e3, entity, i)
            if (i1 != ComponentNull && i2 != ComponentNull && i3 != ComponentNull) return true
        }
        return false
    }
    ...
    inline fun count(): Int {
        var count = 0
        forEach { _, _, _, _, _, _, _ -> count++ }
        return count
    }

    /** Finds the entity with the minimum value selected by [selector]. Returns -1 if no matches. */
    inline fun <R : Comparable<R>> minEntity(crossinline selector: (entity: Int, c1: L1, i1: Int, c2: L2, i2: Int, c3: L3, i3: Int) -> R): Int {
        var minEntity = -1
        var minValue: R? = null
        forEach { entity, c1, i1, c2, i2, c3, i3 ->
            val value = selector(entity, c1, i1, c2, i2, c3, i3)
    ...
    inline fun <R : Comparable<R>> maxEntity(crossinline selector: (entity: Int, c1: L1, i1: Int, c2: L2, i2: Int, c3: L3, i3: Int) -> R): Int {
        var maxEntity = -1
        var maxValue: R? = null
        forEach { entity, c1, i1, c2, i2, c3, i3 ->
            val value = selector(entity, c1, i1, c2, i2, c3, i3)
    ...
    fun shuffle(): Query3<T1, L1, T2, L2, T3, L3> {
    ...
    fun copyEntitiesTo(destination: IntList, clear: Boolean = true): IntList {
        ...
        forEach { entity, _, _, _, _, _, _ -> destination.add(entity) }
        return destination
    }
    ...
    fun copyIndicesTo(destination: IntList, clear: Boolean = true): IntList {
        ...
        forEach { _, _, i1, _, i2, _, i3 ->
            destination.add(if (dp === p1) i1 else if (dp === p2) i2 else i3)
        }
        return destination
    }
    ...
    fun toEntities(): IntList {
        val result = IntList()
        forEach { entity, _, _, _, _, _, _ -> result.add(entity) }
        return result
    }
